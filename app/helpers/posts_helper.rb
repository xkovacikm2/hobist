module PostsHelper
  ...
  def post_search_params(pars)
    pars ||= {}
    pars[:time_to] = pars[:time_to].to_datetime.to_i unless pars[:time_to].nil? or pars[:time_to].empty?
    pars[:time_from] = pars[:time_from].to_datetime.to_i unless pars[:time_from].nil? or pars[:time_from].empty?
